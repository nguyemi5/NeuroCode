...
%         peelingS = zeros(9000,neuronN);
%         peelingC = zeros(9000, neuronN);
        foopsiC = zeros(9000,neuronN);
        foopsiS = zeros(9000,neuronN);
%         mcmcC = zeros(9000, neuronN);
%         mcmcS = zeros(9000, neuronN);
%         timeConstantsF = zeros(neuronN, 1);
%         timeConstantsM = zeros(neuronN, 2);
%         timeConstantsE = zeros(neuronN, 1);
%         [~,~, intY, ~]=APCaModelInt_test(30, 15);
        
        for n=1:neuronN
            neuronDf(n, :) = mean(data(maskStructure(n).WCPixList, :), 1);
        end
    
        parfor n=1:neuronN
%             [APs, ~, ~, ~, ~, ~, ~]=peeling_test(neuronDf(n,:), 30);
%             peelingS(:,n) = APs;
%             peelingC(:,n) = conv(APs, intY, 'same');
%             neuronBase = neuronDf(n,:);
            disp(n)
%             [b, ~] = estimate_baseline_noise(neuronDf(n,:));
              b = baseline_kde(neuronDf(n,:)')';
%             g = estimate_time_constant(neuronBase(1:1000),1);
            neuronDf(n,:) = (neuronDf(n,:)-b)./b*100;
...
%             timeConstantsM(n,:) = options.mcmc_results.tau/30;
        ...
        save(sprintf('%s.mat', filename), 'foopsiC', 'foopsiS', 'neuronDf', 'neuronN')%, 'mcmcC', 'mcmcS', 'peelingS', 'peelingC', 'timeConstantsM', 'timeConstantsF')
