inputVideoFiles = '\\neurodata2\Large data\Video data\MonitoringVideokompHP\cam*\cam*__20210614*';
        ...
        for e=1:length(eegDirectory)
            ...
            if dnMatFile>vDnStart && (dnMatFile + 1/24)<vDnEnd % TODO: add variable .smrx file length
                ...
                if exist(sprintf('%s\\%s\\%s-%s.mp4', outputDirectory, subject, subject, eegDirectory(e).name(ind+2:ind+14)), 'file')
                    fprintf('file %s\\%s\\%s-%s.avi exists - skipping conversion', outputDirectory, subject, subject, eegDirectory(e).name(ind+2:ind+14))
                    continue
                end
                isVideo = false;
